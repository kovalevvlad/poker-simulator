...
def win_count(args):
    my_hand, undealt_cards, opponent_count, table_cards = args
    my_wins = 0
...
def probability_of_winning(my_hand, opponent_count, table_cards):
    ...
    duplicated_cards = [str(card) for card, count in Counter(my_hand + table_cards).items() if count > 1]
    ...
    total_wins = sum(process_pool.map(win_count, [[my_hand, undealt_cards, opponent_count, table_cards]] * cpu_count()))
    return float(total_wins) / float(_ITERATIONS * cpu_count())
